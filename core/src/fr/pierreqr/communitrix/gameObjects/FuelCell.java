...
public class FuelCell extends GameObject {
  ...
  public FuelCell (final Vector size, final int numIndices, final boolean randomize) {
    super           (initDummyModel());
    this.size       = size;
    this.contents   = new int[size.x][size.y][size.z];
  ...
  public void setContents (final Piece piece) {
    noContent   = ( piece.size.x * piece.size.y * piece.size.z )==0;
    size        = piece.size;
    ...
    int yOff  = 0;
    int zOff  = 0;
    for (final Vector p : piece.content) {
        ...
        for (int x=0; x<size.x; ++x) {
          for (int y=0; y<size.y; ++y) {
            for (int z=0; z<size.z; ++z) {
              ...
              if (x==size.x-1 || contents[x+1][y][z]==0) {
                part.rect(  x+radius, y-radius, z-radius,
                            x+radius, y+radius, z-radius,
                            x+radius, y+radius, z+radius,
                            x+radius, y-radius, z+radius,
              ...
              if (y==size.y-1 || contents[x][y+1][z]==0) {
                part.rect(  x+radius, y+radius, z-radius,
                            x-radius, y+radius, z-radius,
                            x-radius, y+radius, z+radius,
                            x+radius, y+radius, z+radius,
              ...
              // Nothing behind it.
              if (z==size.z-1 || contents[x][y][z+1]==0) {
                part.rect(  x+radius, y+radius, z+radius,
                            x-radius, y+radius, z+radius,
                            x-radius, y-radius, z+radius,
                            x+radius, y-radius, z+radius,
